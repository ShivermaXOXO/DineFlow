    ...
    <div className="bg-white rounded-lg shadow border">
      <div className="px-6 py-4 border-b flex justify-between items-center">
        ...
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 transition-colors flex items-center w-full sm:w-auto justify-center"
        >
...
const TableWiseOrders = ({
  ...
  const handleTableClick = (tableNo, hasOrders) => {
    if (hasOrders) {
      const tableWiseKOTs = {};
      savedKOTs.forEach(kot => {
        if (!kot.orderCreated && kot.tableNumber) {
          const t = Number(kot.tableNumber);
          if (!tableWiseKOTs[t]) tableWiseKOTs[t] = [];
          tableWiseKOTs[t].push(kot);
        }
      });
...
const StaffDashboard = () => {
  ...
  const rawEmail = localStorage.getItem('email') || auth?.email || "";
  const staffName = rawEmail.split('@')[0] || auth?.name || "Staff";
  ...
  useEffect(() => {
    const onStorageChange = (event) => {
      if (event.key === "savedOrders") {
        const newValue = JSON.parse(event.newValue || "[]");
        setOrders(newValue); // UI auto refresh
      }
    };
  ...
  useEffect(() => {
    const handler = ({ orders, kots, takeaway }) => {
      ...
      localStorage.setItem("savedKOTs", []);
  ...
  const saveTakeawayOrder = (order) => {
    try {
      ...
      if (orderIndex > -1) {
        ...
        const mergedKots = Array.from(
          new Map(
            [...(oldOrder.kots || []), ...(order.kots || [])]
              .map(kot => [kot.kotId, kot])
          ).values()
        );
      ...
      localStorage.setItem("savedKOTs", JSON.stringify(savedKOTs))
  ...
  const handleAutoPrint = async (order) => {
    try {
      ...
      console.log('Attempting to print bill...', printBillData);
  ...
  const handlePrintBill = async (orderData = null, tableNumber = null, diningType = null, staffId = null) => {
    try {
      const rawEmail = localStorage.getItem('email') || auth?.email || "";
      const derivedName = rawEmail.split('@')[0];
      const serverName = (derivedName || auth?.name || "Staff").toUpperCase();
          ...
          <body>
            <div class="${typeof window !== 'undefined' && window.innerWidth > 768 ? 'print-preview' : ''}">
              <div class="receipt">
                ...
                Server: ${billData.staffName || billData.staff?.name || 'Staff'}
             </div>
                ...
                <div style="margin-top: 10px; border-top: 1px dotted #000; padding-top: 5px;">
                 <span style="font-size: 11px; font-weight: bold;">SERVER: ${serverName}</span>
              </div>
                <div class="separator"></div>
  ...
  const handleCreateOrder = (tableNo = null, isTakeaway = false, existingOrderId = null) => {
    ...
    if (takeawayKOTs.length > 0) {
      ...
      setSavedKOTs(prev =>
        prev.map(k =>
          takeawayKOTs.find(d => d.kotId === k.kotId)
            ? { ...k, orderCreated: true }
            : k
        )
      );
  ...
  const handlePayment = async (total) => {
    try {
      ...
      const currentStaffName = auth.name || localStorage.getItem('name') || 'Staff';
      ...
      const billData = {
        ...
        staffId: parseInt(localStorage.getItem("staffId") || localStorage.getItem("userId") || "1"),
      ...
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/bill/create`, billData);
      setDiscount(0);
      ...
      try {
        if (currentOrderId && !currentOrderId.startsWith('TAKE-')) {
          await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/orders/${currentOrderId}/status`, { status: "completed", finalTotal: billData.finalTotal });
        } else {
      ...
      } catch (error) { toast.warning("Order sync failed."); }

      // Table Cleanup logic... (Same as before)
      if (diningType === 'dine-in' && tableNumber) {
        // ... (Copy your existing cleanup logic here)
        // ...
        // ...
      }
      if (diningType === 'takeaway' && currentOrderId) {
      ...
      // Auto Print
      if (autoPrintEnabled) {
        await handleNativePrint(printData);
      }
  ...
  const handleConfirmPayment = async () => {
    ...
    const taxAmt = addTax ? (total * taxPercentage) / 100 : 0;
    const finalTotal = total + taxAmt;
    const currentStaffName = auth.name || localStorage.getItem('name') || 'Staff Member';
    const payload = {
      hotelId: auth.hotelId,
      staffId: auth.userId,
      staffName: currentStaffName,
    ...
    try {
      ...
      const printData = ({
        ...
        billNumber: billResponse.data.billNumber || 'New'
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              ...
              <div className="p-6">
                ...
                <div className="mb-6 border-b border-gray-200">
                  <nav className="-mb-px flex space-x-8">
                    ...
                    >
                      ...
                      Table Orders ({savedOrders.length})
                    </button>
                    ...
                    <button
                      ...
                      className={`py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'takeaway'
                        ...
                        : 'border-transparent text-gray-500 hover:text-purple-500 hover:border-gray-200'
                        }`}
                    >
                      <FontAwesomeIcon icon={faShoppingBag} className="mr-2" />
                ...
                {activeTab === 'table-orders' && (
                  <>
                    {console.log("🟡 totalTableNumber:", totalTableNumber)}
      ...
      <NewOrderDialog
        ...
        forceDiningType={forceTakeawayMode ? 'takeaway' : null}
        prefillData={prefillOrderData}
        onCreateOrder={handleCreateOrder}
      />

      {showSuccessModal && lastBillData && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-lg p-8 w-full max-w-md text-center shadow-2xl border-4 border-green-500">
            <div className="mb-4">
              <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <FontAwesomeIcon icon={faCheckCircle} className="text-4xl text-green-600" />
              </div>
              <h2 className="text-3xl font-bold text-green-700">Payment Received!</h2>
              <p className="text-gray-500 mt-2">Bill #{lastBillData.billNumber}</p>
              <p className="text-2xl font-bold text-gray-800 mt-2">₹{lastBillData.finalTotal?.toFixed(2)}</p>
            </div>

            <div className="flex flex-col gap-4 mt-6">
              <button
                onClick={() => handleNativePrint(lastBillData)}
                className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold text-xl hover:bg-blue-700 shadow-lg flex items-center justify-center transition-all"
              >
